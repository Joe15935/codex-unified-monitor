...
一个在本机运行的 macOS 菜单栏应用，集中查看 Codex 官方额度、Token、缓存表现、API 等价值，以及套餐容量的长期观察结果。应用采用 Tauri 2 和 SQLite，随应用运行一个官方 Codex app-server 子进程；安装版不启动网页服务器，不收集遥测，也不把数据同步到云端。

**[下载中英双语 v0.2.1（含审计预览功能）](https://github.com/Joe15935/codex-unified-monitor/releases/tag/v0.2.1)** · [套餐额度审计](AUDITOR.zh-CN.md) · [验证记录](TEST_REPORT.md) · [隐私说明（英文）](PRIVACY.md)

## 安装与切换语言

下载 `.dmg` 或 `.app.zip`，将 **Codex Unified Monitor.app** 放到“应用程序”。需要 Apple Silicon Mac 和 macOS 12 或更新版本；此版本未提供 Intel 安装包。先在官方 Codex CLI 或 ChatGPT/Codex 桌面应用中登录，本组件不会要求你输入账号密码。

默认显示中文。点击主界面右上角、设置页或菜单栏小窗中的 **中文 / English** 即可切换；选择会保存在本机，重新启动后仍然生效。主界面、小窗、托盘菜单和随后导出的 HTML 报告都会跟随语言。

点击菜单栏图标可打开小窗。关闭主窗口后监控继续运行；可从小窗或托盘菜单退出。登录时自动启动需要在设置中自行开启。

安装包使用临时签名，**尚未通过 Apple 公证**。首次打开下载的应用时，macOS 可能要求在“隐私与安全性”中批准。不要全局关闭 Gatekeeper，也可按下文从源码构建。
...
安装 Node.js 22.12 或更新版本、Rust stable、Xcode 命令行工具及官方 Codex。
...
构建结果位于 `src-tauri/target/release/bundle/`。开发使用 `npm run tauri dev`，开发服务器只监听 `127.0.0.1`。安装版没有 HTTP 服务器。
...
只读访问 `CODEX_HOME/sessions` 与 `CODEX_HOME/archived_sessions`，默认根目录为 `~/.codex`。组件自己的数据库位于 `~/Library/Application Support/Codex Unified Monitor/monitor.sqlite3`。
