...
- The shutdown correction keeps the event loop running while a background coordinator waits for worker/provider cleanup, addressing the older UI-thread/worker dependency deadlock. A final read-only process check confirmed that the old application and its provider had exited before installation; no force-termination workaround was used.
- **Installed Mac upgrade and data preservation: PASS.** The final v0.3.0 bundle was installed in Applications after preserving the old bundle and a current database backup. Independent verification confirmed all 44,028 existing event rows with every field, 86 quota snapshots and 11 settings keys survived the upgrade. A subsequent observation recorded 602 additional events and one new snapshot. The installed executable SHA256 is `91b00aea44d8400e116a9ee5742829ef7b24632f25b5d81a0ace0df4dcf08eb1`, exactly matching the executable inside the release ZIP.
- **Native Mac controls and cooperative shutdown: PASS.** Chinese overview/settings, English settings and both languages of the Auditor were visually checked. Native weekly quota showed 80% used, a 10,080-minute window and reset timestamp `1789435505`, matching an independent official account read taken near-contemporaneously. Disabling adaptive refresh through the UI persisted across normal Command-Q and relaunch; both the application and its owned provider exited, and adaptive refresh was subsequently restored through the UI. Chinese was restored through the native language control. A final database comparison confirmed every original preference retained its original value; the new defaults are adaptive refresh enabled and low-quota hint 10%. The real Auditor remains INCONCLUSIVE with no controlled declarations or independent baseline fabricated.
- **Native automated background-relaunch painting: unresolved limitation.** One Cua background relaunch displayed a blank or stale window until a resize; a main-thread sample was idle and the database continued receiving official samples at the configured 90-second interval. A settings action reloaded the view to LIVE. Automation did not resolve the regular tray Open Dashboard path. These observations establish continuing collection and working controls/exit, but do not justify an unqualified foreground startup or tray-navigation PASS.
...
Unauthenticated public download verification for [v0.3.1](https://github.com/Joe15935/codex-unified-monitor/releases/tag/v0.3.1): release page and all four assets returned HTTP 200; downloaded contents matched local SHA256 digests. DMG: 2,892,126 bytes; app ZIP: 2,778,064 bytes; Windows installer: 2,176,343 bytes; checksum manifest: 322 bytes. At initial publication, the release was marked prerelease because Windows interactive acceptance and longitudinal Auditor calibration were incomplete.

### Default download correction (2026-09-12)

After the user reported that GitHub still recommended v0.1.0, v0.3.1 was changed from prerelease to the default **Latest** release. This changes distribution metadata only: Windows remains **Beta**, and Tier Auditor remains a **preview** with incomplete longitudinal calibration. No additional platform or classification acceptance is implied. The release title and bilingual opening notice preserve those limits; the v0.1.0 page now links to the newer release. Both READMEs use the permanent `/releases/latest` download entry, with explicit version links for historical releases.

Validation: `gh api repos/Joe15935/codex-unified-monitor/releases/latest` returned `tag_name: v0.3.1`, `prerelease: false`, and `draft: false`. An unauthenticated HTTP request to `/releases/latest` redirected to `/releases/tag/v0.3.1` and returned HTTP 200. Both edited release descriptions matched the prepared notes; release targets and every asset ID, name, size, SHA256 digest and download URL matched their pre-edit metadata. `git diff --check` passed. No application source or installation package changed, so application tests and builds were not repeated for this documentation/metadata correction.
